...
| `merchant-workspace.html` | The **merchant's daily operating console** — the workspace an onboarded merchant lives in. Built around the ecosystem transaction flow **Request → Offer → Settlement → Trust**: an animated four-stage flow ribbon, a **live transaction board** where orders move through the stages, KPI tiles (sales, open orders, escrow-held, trust score), a **Hawil wallet** (balance, escrow, credit line, transfers), **usage-linked financing** offers scored on souk history, a **transparency & trust** panel (identity assertion, verification, escrow, animated trust ring), and an **expatriate-remittance** corridor view converting remittances into direct orders. An **ecosystem modules** row links out to Sudan Express (shipping), **Mowasalaty** (next departures plus a Leaflet/OpenStreetMap route map, opening `mowasalaty.html`) and FinTech (jumps to the financing panel). |
| `fractional.html` | **alSooq × Sudan Express: gold, freight and payments.** A four-tab page on the alSooq brand tokens (sand + clay, IBM Plex, Arabic-first with an English toggle — same tokens as `merchant-workspace.html`). **Gold**: holdings and vault reserves in SGT (1 SGT = 1 gram) with a gram stepper and estimated total. **Freight**: per-kilogram space on Mowasalaty / Sudan Express routes with a weight stepper and cost, paid from the mobile wallet and held in escrow. **USSD**: a working USSD session simulator (placeholder `*888#`) to buy gold, book freight, check the wallet and release escrow from any phone without internet. **Accounts**: a **cross-border stablecoin wallet** (USDT balance, KSA / UAE / Egypt → Sudan corridors, USDT → SDG conversion with fee) and an **e-commerce receivable account** per merchant (account number + payment link, receivables held in escrow, next settlement, recent customer payments). Accessible tabs (arrow keys follow reading direction), deep links `#gold` / `#freight` / `#ussd` / `#accounts`; the workspace's Sudan Express tile opens `#freight`. *Prototype — rates, balances and account numbers are illustrative; nothing is bought, booked, converted or sent.* |
| `accounts.html` | **alSooq accounts landing page (dark edition).** A cinematic black landing page in the alSooq clay/gold palette (IBM Plex, Arabic-first with an English switch): announcement bar, sticky nav with burger menu, hero with trust strip, three-up features, a "Not sure where to start?" banner, a four-card comparison of the alSooq accounts (gold vault, stablecoin wallet, receivable account, USSD access — each linking to its tab in `fractional.html`), a remittance banner, supported currencies, a review carousel (RTL-aware), a two-column FAQ, closing CTA, newsletter and a footer linking the other pages. *Prototype — ratings and reviews are samples.* |
| `merchant-app.html` | **alSooq merchant app hero.** One flooded alSooq-clay field with a black squircle mark, a white signup pill, a black login pill and a round menu button (full-screen menu with the language switch). Three-part hero: two-line headline, a cropped phone showing the merchant app (store header, receivable-account balance with masked number, request/transfer pills, recent USDT / mobile-wallet / USSD payments), and a narrow copy column with one CTA plus a customer link to `accounts.html`. Cream type on clay for contrast (6.6:1). *Figures in the app are illustrative.* |
