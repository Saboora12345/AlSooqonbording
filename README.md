...
| `catalog.html` | The **Scan & Pay catalog** — a bilingual, coupon-style product catalog. Each card carries a **real, scannable QR** encoding an `alsooq.sd/pay` deep-link (verified end-to-end), with was/now pricing in both numeral systems and Sa7 / MTN as proposed payment partners. Linked from the home nav. |
...
The pages link to each other: home → app + dashboard, dashboard → app + home, and the app / onboarding / workspace hand off into the merchant console (`app.html#merchant` / `#merchant:express`).
