...
| `fractional.html` | **alSooq × Sudan Express: gold and freight by the share.** A two-tab page in the Dhabna brand (dark + gold, Cairo / DM Sans, Arabic-first with an English toggle). **Gold vault**: holdings and vault reserves in SGT (1 SGT = 1 gram) with a gram stepper and estimated total. **Shared freight**: per-kilogram space on Mowasalaty / Sudan Express routes with a weight stepper and cost. **USSD & Pay**: a working USSD session simulator (placeholder code `*888#`) that buys gold, books freight, checks the mobile-wallet balance and releases escrow from any phone without internet. Accessible tabs (arrow keys follow reading direction), deep links `#gold` / `#freight` / `#ussd` (the workspace's Sudan Express tile opens `#freight`). *Prototype — prices are illustrative and nothing is bought or booked.* |
